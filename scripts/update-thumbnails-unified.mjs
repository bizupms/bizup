...
const GOLD = "#e8792b";
const BG = "linear-gradient(135deg, #1e5a8d 0%, #1a191d 100%)";
...
.icon-box{width:100px;height:100px;background:rgba(232,121,43,0.12);border:1px solid rgba(232,121,43,0.3);border-radius:20px;display:flex;align-items:center;justify-content:center;margin-bottom:12px}
...
<div class="card">
  ...
  <div class="brand">bizupms.com</div>
  <div class="bar"></div>
</div>
</body></html>`;
}
...
async function main() {
  ...
  for (let i = 0; i < thumbnails.length; i++) {
    ...
    await s3.send(new PutObjectCommand({ Bucket: "bizup-r2", Key: `board/${t.slug}.png`, Body: body, ContentType: "image/png" }));
    console.log(`  업로드 완료`);
