/* ================================================
   BIZUP 대시보드 JavaScript
   ================================================ */

// API URL
const DASHBOARD_API_URL = typeof WORKER_URL !== "undefined" ? WORKER_URL : ""; // TODO: Worker URL 설정 필요
...
async function loadWeeklyChartData(days = 7) {
  ...
  try {
    ...
    const historyResponse = await fetch(
      `${DASHBOARD_API_URL}/history/cached?days=${days}`,
    );
    ...
    const leadsByDate = dateRange.map((dateStr) => {
      return leads.filter(
        (l) => l.createdTime && l.createdTime.startsWith(dateStr),
      ).length;
    });
